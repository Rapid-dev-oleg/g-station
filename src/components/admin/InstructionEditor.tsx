        ...
        return (
          <Card key={sec.key} title={sec.label} subtitle={sec.hint}
            action={instr
              ...
              : <Button size="sm" variant="secondary" disabled={busy} onClick={() => run(() => ensureSection(typeCode, sec.key))}>Добавить раздел</Button>}>
            {!instr && <span style={{ color: '#aaa', fontSize: 13 }}>Раздел не заведён.</span>}
