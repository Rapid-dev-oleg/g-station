...
function priceMainPump(
  ...
  // Не нашли точно — даём ту же оценку, что и без каталога (избегаем price=0,
  // иначе позиция выпадает из ИТОГО и в смете появляется ложный 0 ₽).
  return {
    price: ESTIMATE_PUMP_USD,
    currency: 'USD',
    note: 'грубая оценка — типоразмер не найден в каталоге, цену уточнить',
  };
}
...
function priceCollector(
  ...
  // Не нашли — даём такую же оценку, что и без каталога (избегаем price=0,
  // иначе позиция «коллектор 0 ₽» выпадает из ИТОГО и портит смету).
  return {
    price: ESTIMATE_COLLECTOR_RUB,
    note: 'грубая оценка — точная цена коллектора не найдена в каталоге',
  };
}
