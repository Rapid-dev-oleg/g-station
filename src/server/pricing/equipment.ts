...
// ──────────────────────── UPSERT из веба в catalog ────────────────────────

/**
 * Бренд из модели типа «CNP CDM42-7FSWPC» / «Wilo SCP 80-160».
 * Возвращает имя производителя или 'Веб-каталог' если не распознали.
 */
function extractManufacturer(model: string | undefined | null): string {
  const m = (model ?? '').trim();
  const known = ['CNP', 'Wilo', 'Grundfos', 'Wellmix', 'Pedrollo', 'DAB', 'Espa', 'Ebara'];
  for (const k of known) {
    if (new RegExp(`^\\s*${k}\\b`, 'i').test(m)) return k;
  }
  return 'Веб-каталог';
}

/**
 * Записать в catalog найденный вебом насос. Используется когда lookup в БД
 * не нашёл SKU — следующий расчёт того же SKU возьмёт из БД мгновенно.
 *
 * Идемпотентно: повторный вызов с тем же (manufacturer, sku) обновит цену/дату.
 */
export async function upsertPumpFromWeb(args: {
  model?: string | null;
  article?: string | null;
  ...
  const sku = (args.article ?? args.model ?? '').replace(/^\s*(CNP|Wellmix|Wilo)\s+/i, '').trim();
  if (!sku || args.priceRub == null) return { ok: false };

  const manufacturerName = extractManufacturer(args.model ?? args.article);
  const manuf = await db.manufacturer.upsert({
    where: { name: manufacturerName },
    update: {},
    create: { name: manufacturerName, note: 'веб-источник' },
  });

  // Проверим — есть ли уже в БД (вдруг под другим SKU-нормализованным)?
  const existing = await db.catalogItem.findUnique({
    where: { manufacturerId_sku: { manufacturerId: manuf.id, sku } },
  });
  const attrs = {
    ...(typeof existing?.attributes === 'object' && existing.attributes !== null
      ...
      : {}),
    source: 'web',
    web_supplier: args.supplier ?? null,
    web_note: args.note ?? null,
