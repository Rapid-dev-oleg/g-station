...
/**
 * Server actions для систем — мутации. Изолировано по воркспейсу.
 *
...
/** Создаёт систему с пустым делом (проект должен быть в активном воркспейсе). */
export async function createSystem(input: CreateSystemInput) {
  const db = await workspaceDb();
  const project = await db.project.findUnique({ where: { id: input.projectId } });
  if (!project) throw new Error('Проект не найден в вашем воркспейсе');
...
/** Переименовывает систему (только в своём воркспейсе). */
export async function renameSystem(systemId: string, name: string) {
  const db = await workspaceDb();
  const system = await db.system.findUnique({ where: { id: systemId } });
  if (!system) return { id: systemId };
  await db.system.updateMany({ where: { id: systemId }, data: { name } });
  revalidatePath(`/systems/${systemId}`);
  revalidatePath(`/projects/${system.projectId}`);
  return { id: systemId };
}

/** Удаляет систему (только в своём воркспейсе). */
export async function deleteSystem(systemId: string) {
  const db = await workspaceDb();
  const system = await db.system.findUnique({ where: { id: systemId } });
  if (!system) return { id: systemId };
  await db.system.deleteMany({ where: { id: systemId } });
  revalidatePath(`/projects/${system.projectId}`);
  return { id: systemId };
}
